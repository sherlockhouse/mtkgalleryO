...
public class MoviePlayer implements MediaPlayerWrapper.Listener,
    ...
    private void doOnPause() {
        ...
        // / M: Android O change surface do not belong to WMS.
        // It will not be set to invisible when activity finished.
        // so, we set it invisible when acitivty invisible.
    ...
    private void getVideoInfo(MediaPlayer mp) {
        ...
        if (!MovieUtils.isLocalFile(mMovieItem.getVideoType())) {
            //int timeout = VideoMetadataUtils.getServerTimeout(mp);
            int timeout = SystemPropertyUtils.getInt("sys.rtsp.server.timeout", DEFAULT_SERVER_TIMEOUT);
            Log.v(TAG, "getVideoInfo() rtsp timeout =" + timeout);
            if (timeout != -1) {
                mServerTimeoutExt.setTimeout(timeout);
            } else {
                Log.w(TAG, "timeout is invalid!");
            }
