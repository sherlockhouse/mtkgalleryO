...
public class VideoMetadataUtils {
    ...
    public static int getServerTimeout(MediaPlayer mp) {
        int timeout = -1;
        if (isMetadataSupported()) {
            long checkStart = SystemClock.elapsedRealtime();
            final Metadata data = mp.getMetadata(MediaPlayer.METADATA_ALL,
                    MediaPlayer.BYPASS_METADATA_FILTER);
            if (data != null && data.has(Metadata.SERVER_TIMEOUT)) {
                timeout = data.getInt(Metadata.SERVER_TIMEOUT);
                Log.d(TAG, "getServerTimeout, timeout = " + timeout);
            }
            if (DEBUG) {
                Log.d(TAG, "getServerTimeout, getMetadata elapsed time = "
                        + (SystemClock.elapsedRealtime() - checkStart));
            }
        }
        return timeout;
