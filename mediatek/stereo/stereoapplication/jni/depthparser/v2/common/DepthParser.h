...
public:
    DepthParser(DepthBuf* depthBuf) {
        LOGD("<DepthParser><comp> new DepthParser (v2)");
    ...
    MUINT8* getOccMap() {
        LOGD("<getOccMap><comp> (v2)");
        if (m_pDepthBuf == NULL) {
            LOGD("<getOccMap> m_pDepthBuf invalid, return NULL!!");
            return NULL;
        }
        MUINT8* pOccBuf = NULL;
        MINT32 len = m_pDepthBuf->metaWidth * m_pDepthBuf->metaHeight;
        if (len <= 0) {
            LOGD("<getOccMap> len <= 0, return NULL!!");
            return NULL;
        }
        pOccBuf = new MUINT8[len];
        MINT32 offset = m_pDepthBuf->depthWidth * m_pDepthBuf->depthHeight +
                3 * m_pDepthBuf->metaWidth * m_pDepthBuf->metaHeight;
