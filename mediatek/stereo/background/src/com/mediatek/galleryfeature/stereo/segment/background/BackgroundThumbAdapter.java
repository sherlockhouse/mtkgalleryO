...
class BackgroundThumbAdapter extends ArrayAdapter<BackgroundThumbAction> {
    private static final String LOGTAG = Log.Tag("Bg/BackgroundThumbAdapter");
    ...
    private BackgroundDBHelper dbHelper;
    ...
    public BackgroundThumbAdapter(Context context, int textViewResourceId) {
        ...
        dbHelper = new BackgroundDBHelper(context);
    }
    ...
    public void loadActions(StereoBackgroundActivity activity, int verticalItemHeight) {
        ...
        try {
            ...
            Log.e(LOGTAG, "<loadActions> query db fail, ", se);
        }
    ...
    public void saveActions() {
        ...
        try {
            ...
            Log.d(LOGTAG, "<saveActions> db exception", se);
        }
        db.close();
    }
    ...
    public void remove(BackgroundThumbAction action) {
        super.remove(action);
        Log.d(LOGTAG, "<remove> " + action.getUri());
        action.clearBitmap();
    ...
    class BackgroundDBHelper extends SQLiteOpenHelper {

        private static final int DATABASE_VERSION = 1;
        private static final String DATABASE_NAME = "bg.db";
        private static final String SQL = "create table recent_bg (" +
                "_id integer primary key autoincrement, img_path varchar(255))";

        public BackgroundDBHelper(Context context) {
