...
public class ReFocusView extends StageView {
    private static final String TAG = Log.Tag("Rf/ReFocusView");
    ...
    // workaround a3m issue on Mali-G71 driver
    private boolean mIsWorkaround = true;
    private Bitmap mDummyActorBitmap;
    private Image mDummyActor;
    ...
    private boolean mResetIsShowImageIsTransitionPlaying = false;
    ...
    private float mMaxAnimationLevel;
    ...
    /**
     * Set max animation level.
     * @param level max level
     */
    public void setAnimationLevel(float level) {
        mMaxAnimationLevel = level;
        logD("setAnimationLevel(): " + level);
    }
    ...
    protected synchronized void onDetachedFromWindow() {
        ...
        // workaround a3m issue on Mali-G71 driver
        if (null != mDummyActorBitmap)
        {
            mDummyActorBitmap.recycle();
            mDummyActorBitmap = null;
        }

        for (Object key : mRecycleMap.keySet()) {
            logD("Recycled: " + key + " x" + mRecycleMap.get(key));
        }
        mRecycleMap.clear();
    }
    ...
    public synchronized void setImageActorNew(Object pic, boolean recycle) {
        ...
        if (pic != null) {
            ...
            // 2nd time to set new Actor (algorithm triggered by touch event)
            // check animation playing or not
            // playing: set to newActor
            // not playing: set to Actor
            if (mBitmapNew != null && !mIsShowImageIsTransitionPlaying) {
                setImageActor(pic, -1, -1);
                return;
            }
    ...
    public synchronized void onDrawFrame(GL10 gl) {
        ...
        if (mResetIsShowImageIsTransitionPlaying) {
            mResetIsShowImageIsTransitionPlaying = false;
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_UP:
                    ...
                    logI("onTouchEvent() @View(" + event.getX() + ","
                            + event.getY() + ") " + mIsShowImageIsTransitionPlaying);

                    if (!mIsShowImageIsTransitionPlaying &&
                        mImageActor.hitTest(new Point(mDownX, mDownY)) != null) {
                        Log.d(TAG, "mDownX = " + mDownX + "|| mDownY = "
                                + mDownY);
    ...
    private void onDrawFrameImage() {
        ...
        mImageActor.setMaterialProperty("", "M_LEVEL", mMaxAnimationLevel);
        ...
        if (isEnd) {
            ...
            mResetIsShowImageIsTransitionPlaying = true;
        }
    }
    ...
    public synchronized void moveBackCheck() {
    ...
    private synchronized void setImageActor(Object pic, boolean recycle) {
        logI("setImageActor(" + recycle + ")" + pic);
        long begin = System.currentTimeMillis();
        long spentTime = 0;
        if (pic != null) {
            if (mBitmap != null && recycle) {
                if (pic == mBitmap) {
                    logW("ignore same instance " + pic);
                    return;
                }
                recycle(mBitmap);
                logD("********* mBitmap.recycle()************");
            }
            spentTime = System.currentTimeMillis() - begin;
            logD("setImageActor mBitmap.recycle SpentTime = " + spentTime);
            mBitmap = pic;
            begin = System.currentTimeMillis();
            setTextureBase(mImageActor, mBitmap);
            spentTime = System.currentTimeMillis() - begin;
            logD("mImageActor.setImageFromBitmap spentTime = " + spentTime);
            mImageActor.setSize(getDimension(mBitmap));
            logD(" mImageActor.setImageFromBitmap(mBitmap)");
            setTextureNew(mImageActor, mBitmapNew);
            mImageActor.setMaterialProperty("", "M_STEP", 0.0f);
            mImageActor.setMaterialProperty("", "M_FADEOUTSTEP", 1.0f);
        }
    ...
    private void setup() {
        ...
        // workaround a3m issue on Mali-G71 driver
        if (mIsWorkaround) {
            mDummyActor = Image.createEmptyImage();
            mDummyActor.setPosition(
                new Point(-3f * mDepthLocationX, -3f * mDepthLocationY, -DEPTH_ACTOR_Z, true));
            mDummyActor.setSize(new Dimension(DEPTH_ACTOR_SIZE, DEPTH_ACTOR_SIZE));
            mDummyActor.setVisible(true);
            mStage.add(mDummyActor);
        }
    }

    private static HashMap<String, Integer> mRecycleMap = new HashMap<String, Integer>();

    private static void recycle(Object pic) {
        if (pic != null) {
            String mykey = "r_" + pic + "";
            if (mRecycleMap.containsKey(mykey)) {
                Integer count = (Integer)mRecycleMap.get(mykey);
                mRecycleMap.put(mykey, (count+1));
                Log.d(TAG, "" + pic + " has been recycled x" + count);
                return;
            }
            Log.d(TAG, "recycle() " + pic);
            mRecycleMap.put(mykey, 1);
            if (pic instanceof Bitmap) {
                ((Bitmap) pic).recycle();
            } else {
                AndroidGraphicBuffer.destroy(pic);
            }
        }
    }
    ...
    private static float getWidth(Object pic) {
        if (pic instanceof Bitmap) {
            ...
            return AndroidGraphicBuffer.getWidth(pic);
        }
    }
    ...
    private static float getHeight(Object pic) {
        if (pic instanceof Bitmap) {
            ...
            return AndroidGraphicBuffer.getHeight(pic);
        }
    }
    ...
    private static Dimension getDimension(Object pic) {
        if (pic instanceof Bitmap) {
            ...
            return new Dimension(AndroidGraphicBuffer.getWidth(pic), AndroidGraphicBuffer
                    .getHeight(pic));
        }
    }

     private static Bitmap resizeBitmap(Bitmap img) {
        int finalWidth = img.getWidth() / 2;
        int finalHeight = img.getHeight() / 2;
    ...
    private synchronized void setTextureBase(Image img, Object pic) {
        ...
        if (pic instanceof Bitmap) {
            ...
            // workaround a3m issue on Mali-G71 driver
            if (mIsWorkaround && null == mDummyActorBitmap)
            {
                mDummyActorBitmap = resizeBitmap((Bitmap)pic);
                mDummyActor.setImageFromBitmap(mDummyActorBitmap);
            }
        } else {
            img.setGraphicBufferImage(pic);
    ...
    private synchronized void setTextureNew(Image img, Object pic) {
        ...
        if (pic instanceof Bitmap) {
            ...
            img.setMaterialProperty("", "M_NEW_TEXTURE", pic);
