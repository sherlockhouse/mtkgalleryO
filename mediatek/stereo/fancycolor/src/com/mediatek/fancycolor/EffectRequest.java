 ...
 */
class EffectRequest implements ThreadPool.Job<Void> {
    private final static String TAG = Log.Tag("Fc/EffectRequest");
    ...
    public EffectRequest(FancyColor fancyColor, int index, String effectName, Bitmap bitmap,
                         int type, EffectRequestListener listener) {
        mEffectName = effectName;
        mFancyColor = fancyColor;
        mIndex = index;
        mOriBitmap = bitmap;
        mType = type;
        mListener = listener;
    ...
    public Void run(ThreadPool.JobContext jc) {
        ...
        TraceHelper.beginSection(">>>>FancyColor-EffectRequest");
        Bitmap bitmap = mFancyColor.getFancyColorEffectImage(mOriBitmap, mEffectName, mType);
        long cost = System.currentTimeMillis() - start;
        Log.d(TAG, "<EffectRequest.run> index & effectName & type & cost: " +
                mIndex + " " + mEffectName + " " + mType + " " + cost);
