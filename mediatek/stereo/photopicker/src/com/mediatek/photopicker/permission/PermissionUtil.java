package com.mediatek.photopicker.permission;
...
/**
 * Check and request permission for PhotoPicker.
 */
public class PermissionUtil {
    private static final String TAG = "PhotoPicker/PermissionUtil";
    ...
    public static void showDeniedPrompt(Context context) {
        Toast.makeText(context, getDeniedPermissionString(context), Toast.LENGTH_SHORT).show();
    }

    private static String getDeniedPermissionString(Context context) {
        if (!sHasChecked) {
            try {
                sDeniedPermissionString = context.getResources().getString(Utils.getMyResId(context,
                        "photopicker_permission_deny", "string"));
            } catch (Exception e) {
