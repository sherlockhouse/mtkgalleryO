...
public class StereoSegmentWrapper {
    private static final String TAG = Log.Tag("Cp/StereoSegmentWrapper");
    ...
    private final ImageSegment mSegment;
    ...
    private class MySecretary {
        ...
        private class DoSegment implements Callable<Boolean> {
            ...
            public Boolean call() throws Exception {
                Log.d(TAG, "<DoSegment_call> segment.doSegment() start");
                TraceHelper.beginSection(">>>>SegmentJni-doSegment");
                ...
                Log.d(TAG, "<DoSegment_call> segment.doSegment() end");
                if (result) {
        ...
        public boolean initSegment(final Uri sourceUri, final String sourceFilePath,
            ...
            Callable<Boolean> initOp = new Callable<Boolean>() {
                @Override
                public Boolean call() throws Exception {
                    Log.d(TAG, "<initSegment> initSegment() start");
                    TraceHelper.beginSection(">>>>SegmentJni-initSegment");
                    boolean isSegmentValid = mSegment.initSegment(sourceUri, sourceFilePath,
                            bitmap, 0);
                    TraceHelper.endSection();
                    Log.d(TAG, "<initSegment> initSegment() end");

                    if (isSegmentValid) {
                        Log.d(TAG, "<initSegment> generateSegmentMask() start");
                        TraceHelper.beginSection(">>>>SegmentJni-doSegment-autoPick");
                        isSegmentValid = mSegment.doSegment(ImageSegment.SCENARIO_AUTO,
                                ImageSegment.MODE_OBJECT, null, null, null);
                        TraceHelper.endSection();
                        Log.d(TAG, "<initSegment> generateSegmentMask() end");
                        if (isSegmentValid) {
                            invalidateMaskBitmap();
                        }
                    }
        ...
        public void releaseSegment() {
            ...
            Callable<Void> releaseOp = new Callable<Void>() {
                ...
                public Void call() throws Exception {
                    Log.d(TAG, "<releaseSegment> segment.release() start");
                    TraceHelper.beginSection(">>>>SegmentJni-release");
                    mSegment.release();
                    TraceHelper.endSection();
                    Log.d(TAG, "<releaseSegment> segment.release() end");
                    return null;
                }
            };
        ...
        public boolean undoSegment() {
            ...
            Callable<Boolean> op = new Callable<Boolean>() {
                @Override
                public Boolean call() throws Exception {
                    TraceHelper.beginSection(">>>>SegmentJni-undoSegment");
        ...
        private void invalidateMaskBitmap() {
            Log.d(TAG, "<invalidateMaskBitmap> mSegment.getCoverBitmap() start");
            TraceHelper.beginSection(">>>>SegmentJni-getMaskBitmap");
            mMaskBitmap = mSegment.getCoverBitmap();
            TraceHelper.endSection();
            Log.d(TAG, "<invalidateMaskBitmap> mSegment.getCoverBitmap() end");
        }
    }
     ...
     *            never used. TODO remove.
     */
    public StereoSegmentWrapper(Context context, int width, int height, int widthSmall,
                                int heightSmall) {
        Log.d(TAG, "<StereoSegmentWrapper> Segment() start");
        TraceHelper.beginSection(">>>>SegmentJni-constructor");
        mSegment = new ImageSegment(context);
        TraceHelper.endSection();
        Log.d(TAG, "<StereoSegmentWrapper> Segment() end");
            ...
            public void onSegmentOperationDone(final int scenario, final boolean result) {
                ...
                mMainHandler.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (scenario == ImageSegment.SCENARIO_SELECTION
                                || scenario == ImageSegment.SCENARIO_AUTO) {
                            mRefineSteps = 1;
     ...
     *
     * @param sourceUri
     *            source Uri of image to segment.
     * @param filePath
     ...
     */
    public boolean initSegment(Uri sourceUri, String filePath, Bitmap inBitmap) {
        if (sourceUri == null || filePath == null || inBitmap == null) {
            return false;
        }
        Log.d(TAG, "<initSegment> filePath = " + filePath + ",width:" + inBitmap.getWidth()
                + ",height:" + inBitmap.getHeight());
        boolean isSegmentValid = true;

        mInBitmapWidth = inBitmap.getWidth();
        mInBitmapHeight = inBitmap.getHeight();
        isSegmentValid = mSecretary.initSegment(sourceUri, filePath, inBitmap, 0);
        mRefineSteps = 1;
    ...
    public float[] getSegmentPoint() {
        ...
        TraceHelper.beginSection(">>>>SegmentJni-getSegmentPoint");
    ...
    public void addSelection() {
        ...
        info.mScenario = ImageSegment.SCENARIO_SCRIBBLE_FG;
        info.mMode = 0;
    ...
    public void delSelection() {
        ...
        DoSegmentInfo info = new DoSegmentInfo();
        info.mScenario = ImageSegment.SCENARIO_SCRIBBLE_BG;
        info.mMode = 0;
        info.mScribble = mRefineBitmap;
        info.mRoiRect = roiRect;
        info.mPoint = null;
    ...
    public void setMaskCenter(int x, int y) {
        ...
        y = y * mInBitmapHeight / mOriginalHeight;
        DoSegmentInfo info = new DoSegmentInfo();
        info.mScenario = ImageSegment.SCENARIO_SELECTION;
        info.mMode = 0;
        info.mScribble = null;
        info.mRoiRect = null;
    ...
    public void autoSegment() {
        DoSegmentInfo info = new DoSegmentInfo();
        info.mScenario = ImageSegment.SCENARIO_AUTO;
        info.mMode = 0;
        info.mScribble = null;
        info.mRoiRect = null;
        info.mPoint = null;
    ...
    public Rect getClippingBox() {
        TraceHelper.beginSection(">>>>SegmentJni-getSegmentRect");
    ...
    public Bitmap getForground(Bitmap oriBitmap) {
        ...
        if ((oriBitmap == null) || (oriBitmap == mInitBitmap) ||
                (mInitBitmap != null
                        && (mInitBitmap.getWidth() == oriBitmap.getWidth()
                        || mInitBitmap.getHeight() == oriBitmap.getHeight()))) {
            TraceHelper.beginSection(">>>>SegmentJni-getSegmentBitmap");
            bitmap = mSegment.getSegmentBitmap(mInitBitmap);
            TraceHelper.endSection();
        } else {
            TraceHelper.beginSection(">>>>SegmentJni-setNewBitmap");
            mSegment.setNewBitmap(oriBitmap, null, 0, 0);
            TraceHelper.endSection();
            TraceHelper.beginSection(">>>>SegmentJni-getNewSegmentBitmap");
            bitmap = mSegment.getNewSegmentBitmap(oriBitmap);
            Log.d(TAG, "<getForground> new forceground: bitmap = " + bitmap);
            if (bitmap != null) {
                Log.d(TAG, "<getForground> new forceground: w = " + bitmap.getWidth() + "|" + bitmap
                        .getHeight());
            }
            TraceHelper.endSection();
        }
     ...
     */
    public boolean undo() {
        Log.d(TAG, "<undo> mRefineSteps = " + mRefineSteps);
        if (mRefineSteps > 1) {
            mRefineSteps--;
    ...
    public void saveMaskToXmp() {
        TraceHelper.beginSection(">>>>SegmentJni-saveMaskToXmp");
    ...
    public void release() {
        ...
        if (mRefineBitmap != null && !mRefineBitmap.isRecycled()) {
    ...
    private Rect getRoiRect() {
        ...
        int right = Math.min((int) (pathAABBBox.right) + 1, mInBitmapWidth);
        int top = Math.max((int) (pathAABBBox.top) - 1, 0);
        int bottom = Math.min((int) (pathAABBBox.bottom) + 1, mInBitmapHeight);
        Rect roiRect = new Rect(left, top, right, bottom);
        Log.d(TAG, "<getRoiRect> rotRect(" + left + ", " + top + " - " + right + ", " + bottom
                + ")");
        return roiRect;
    }

    private void regenerateRefinePath(int bgColor, int pathColor) {
        TraceHelper.beginSection(">>>>StereoSegmentWrapper-regenerateRefinePath");
