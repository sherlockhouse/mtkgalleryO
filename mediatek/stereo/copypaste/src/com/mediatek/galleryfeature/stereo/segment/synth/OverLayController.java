...
public class OverLayController {
    private static final String TAG = Log.Tag("Cp/OverLayController");
    ...
    private final int mActionBarHeight = 40;
    private final int mDisPlayBounder = 20;
    ...
    public boolean getTouchEvent(MotionEvent event) {
        ...
        Log.d(TAG, " pointF[0] = " + pointF[0] + " pointF[1] = " + pointF[1]
    ...
    public void drawBitmap(Canvas canvas) {
        ...
        if (!OverLayerDrawingUtils.setImageToScreenMatrix(saveMatrix,
            ...
            Log.w(TAG, "<saveBitmap>failed to get screen matrix");
        }
        ...
        if (cropBitmap != null && !mHasDrawCropBitmap
            ...
            RectF mIconRectBounds = new RectF(
                    ...
                    backgroundBitmap.getHeight() / 2 + cropBitmap.getHeight()
                            / 2);
            mBackgroundRect =
                    new RectF(0, mActionBarHeight, backgroundBitmap.getWidth(),
                            backgroundBitmap.getHeight());
            initialize(null, cropBitmap, mIconRectBounds, mBackgroundRect, 0);
    ...
    private void resetOuter(RectF innerBounds) {
        if (mCropObj != null) {
            int bound = (int) Math.min(innerBounds.width(), innerBounds.height()) / 3;
            float left = mBackgroundRect.left - bound + mDisPlayBounder;
            float top = mBackgroundRect.top - bound + mDisPlayBounder;
            float right = mBackgroundRect.right + bound - mDisPlayBounder;
            float bottom = mBackgroundRect.bottom + bound - mDisPlayBounder;
            RectF outer = new RectF(left, top, right, bottom);
            mCropObj.resetOuter(outer);
        }
    }
}
