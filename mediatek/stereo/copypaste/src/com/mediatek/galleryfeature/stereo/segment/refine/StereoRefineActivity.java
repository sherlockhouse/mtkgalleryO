...
import com.mediatek.galleryfeature.stereo.segment.Utils;
...
public class StereoRefineActivity extends Activity {
    private static final String TAG = Log.Tag("Cp/StereoRefineActivity");
    ...
    private static final String ACTION_STEREO_REFINE = "com.mediatek.segment.action.REFINE";
    ...
    private AtomicBoolean mSavingMask = new AtomicBoolean(false);
    /**
    ...
    private class LoadBitmapTask extends AsyncTask<Uri, Boolean, Boolean> {
        ...
        protected void onPostExecute(Boolean result) {
            ...
            if (mMaskSimulator != null) {
                mImageShowPick.setSegmenter(mMaskSimulator);
                mImageShowZoomPan.setSegmenter(mMaskSimulator);
                mImageShowRefine.setSegmenter(mMaskSimulator);
    ...
    private void updateUIByUIMode() {
        if (mMenu == null || mUIMode == UI_MODE_NONE) {
            return;
        }
        switch (mUIMode) {
        case UI_MODE_PICK:
    ...
    private String saveRefineResult() {
        ...
        final File folder = new File(Environment.getExternalStorageDirectory()
                .getAbsolutePath().toString() + "/" + STEREO_CLIPPINGS_FOLDER_NAME);
        if (!folder.exists()) {
            Log.d(TAG, "<saveRefineResult> create Clippings folder");
            folder.mkdir();
        }
                ...
                runOnUiThread(new Runnable() {
                    public void run() {
                        ...
                        Toast.makeText(StereoRefineActivity.this,
                                ...
                                .show();
                        // While finish saveCliping, set AtomicBoolean as false for next
                        // save clipping operation.
                        mSavingMask.set(false);
                    }
                });
            }
    ...
    private void configButtons() {
        ...
        mButtonSaveClipping.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mSavingMask.compareAndSet(false, true)) {
                    saveRefineResult();
                }
            }
        });
