...
public class FocusIndicator {
    private final static String TAG = Log.Tag("Cp/FocusIndicator");
    ...
    public void onSingleTapUp(int ex, int ey) {
        Log.d(TAG, "<onSingleTapUp> ex:" + ex + ",ey:" + ey);
        ...
        Log.d(TAG, "<onSingleTapUp> width:" + width + ",height:" + height);

        int left = Math.max(0, ex - width / 2);
        int top = Math.max(0, ey - height / 2);
        p.setMargins(left, top, 0, 0);

        Log.d(TAG, "<onSingleTapUp> left:" + left + ",top:" + top);
